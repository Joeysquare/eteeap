...
router.post("/", cpUpload, async (req, res) => {
  try {
    ...
    // Create admin_notifications table if not exists and insert notification for admins
    try {
      await db.query(
      ...
      const noteTitle = 'New Application';
      const noteMessage = `New application submitted by ${body.full_name || 'a user'} for ${body.program_name || 'a program'}`;
      await db.query(`INSERT INTO admin_notifications (application_id, user_id, title, message, created_at) VALUES (?, ?, ?, ?, NOW())`,
        [result.insertId, String(userId), noteTitle, noteMessage]
      );
    } catch (nerr) {
