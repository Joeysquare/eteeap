...
function Notifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      if (user.role === 'admin') {
        const res = await axios.get("http://localhost:5000/notifications/admin", { headers: { "x-user-id": user.id }, withCredentials: true });
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          if (n.id) await axios.post(
                            "http://localhost:5000/notifications/admin/mark-read",
                            { id: n.id },
                            { headers: user ? { 'x-user-id': user.id } : {}, withCredentials: true }
                          );
                        } catch (e) {
                          console.error("Admin mark read failed:", e?.response?.data || e.message);
                        }
                        if (n.application_id) navigate(`/admin?application_id=${n.application_id}`);
                        else navigate('/admin');
                      }}
