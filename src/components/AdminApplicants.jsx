...
import { Trash2, Check, XCircle, Download, Eye } from "lucide-react";
...
function AdminApplicants() {
  ...
  const exportCSV = async () => {
    const rows = [["Full Name", "Email", "Phone", "Program", "Status"]];
    applicants.forEach(a => rows.push([a.full_name, a.email, a.phone || "", a.program_name, a.status]));
    ...
    await logActivity("Export CSV", `Exported ${rows.length - 1} applicants`);
  };
  ...
  const filtered = applicants.filter(a => {
    ...
    const matchesProgram = programFilter === 'All' ? true : (a.program_name === programFilter);
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-2">
        <h2 className="text-xl font-semibold text-blue-800">Applicants</h2>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {Array.from(new Set(applicants.map(a => a.program_name).filter(Boolean))).sort().map(p => (
            ...
            {filtered.map(a => (
              <tr key={a.id} className="border-b hover:bg-gray-50 cursor-pointer"
                ...
                <td className="p-2 text-center flex justify-center gap-2"
                  ...
                  <button onClick={() => acceptRejectApplicant(a.id, "Accepted")}
                    className="text-green-600 hover:text-green-800 px-2 py-1 rounded bg-green-50">
                    Accept
                  </button>
                  <button onClick={() => acceptRejectApplicant(a.id, "Rejected")}
                    className="text-red-600 hover:text-red-800 px-2 py-1 rounded bg-red-50">
                    Reject
                  </button>
                  <button onClick={() => acceptRejectApplicant(a.id, "Pending")}
                    className="text-yellow-600 hover:text-yellow-800 px-2 py-1 rounded bg-yellow-50">
                    Pending
                  </button>
